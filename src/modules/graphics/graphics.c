...
void lovrGraphicsDestroy() {
  ...
  for (uint32_t i = 0; i < MAX_DEFAULT_SAMPLERS; i++) {
    lovrRelease(Sampler, state.defaultSamplers[i]);
  }
...
void lovrGraphicsSetShader(Shader* shader) {
  ...
  lovrRetain(shader);
  lovrRelease(Shader, thread.shader);
...
Sampler* lovrSamplerCreate(SamplerInfo* info) {
  Sampler* sampler = _lovrAlloc(sizeof(sampler) + gpu_sizeof_sampler());
